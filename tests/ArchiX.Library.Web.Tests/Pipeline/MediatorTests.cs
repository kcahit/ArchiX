...
namespace ArchiX.Library.Web.Tests.Pipeline
{
 ...
 {
 public Task<string> HandleAsync(MReq request, CancellationToken ct) => Task.FromResult($"[{request.Msg}]");
 }

 public sealed class MHandler2 : IRequestHandler<MReq, string>
 {
 public Task<string> HandleAsync(MReq request, CancellationToken ct) => Task.FromResult($"2[{request.Msg}]");
 }
 ...
 private static Exception Unwrap(Exception e) => e is TargetInvocationException tie && tie.InnerException is not null ? tie.InnerException : e;
 ...
 var sp = BaseSvc(s =>
 {
 s.AddTransient<IRequestHandler<MReq, string>>(_ => new InlineHandler((_, ct) =>
 {
