...
using Xunit;
using IUnitOfWork = ArchiX.Library.Abstractions.Persistence.IUnitOfWork;
...
namespace ArchiX.Library.Web.Tests.Pipeline
{
 ...
 var descriptors = s
 .Where(sd => sd.ServiceType.IsGenericType && sd.ServiceType.GetGenericTypeDefinition() == typeof(IPipelineBehavior<,>))
 .ToArray();
